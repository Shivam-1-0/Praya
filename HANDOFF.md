...
| `(admin)/*` | **Fully built (Phase 9).** `requireAdmin()` guard in `src/lib/admin/guard.ts` (anon client checks `is_admin`, redirects otherwise). Pages: `/admin` overview stats, `/admin/users` list, `/admin/users/[id]` per-user overview, `/admin/audit` last 100 audit rows. Cross-user reads go through `src/lib/admin/queries.ts` (service-role) and every one writes `admin_audit_log`. First admin is bootstrapped in SQL: `update profiles set is_admin=true where user_id='<uuid>'` — no promote/demote UI on purpose. |
...
2. **Review edit window — RESOLVED.** Reviews are editable for 12 hours after the FIRST submit, then permanently locked. `LOCK_HOURS`, `isReviewLocked()`, `reviewLocksAt()`, and `timeUntilLock()` live in `src/lib/review-lock.ts` (runnable check: `src/lib/review-lock.check.ts`). Enforcement: `submitReview` rejects submits when `isReviewLocked(existing.completed_at)`. `completed_at` is preserved from the original submit on re-submits so the 12h clock never resets. UI: `ReviewClient` shows a "Locked" banner when past the window (fieldset disabled, submit button reads "Locked"), or a "Editable for another Xh Ym, then locked" chip while the window is open.
...
5. **Habit reordering — RESOLVED.** `sort_order` is now live. Habits page and Today both order by `sort_order` then `created_at` (existing rows are all `sort_order=0`, so the created_at tiebreak preserves prior order until a user reorders). Up/down arrow buttons on each habit card (`HabitsClient.tsx`) do optimistic local swaps and persist via `reorderHabits(orderedIds)` in `habits/actions.ts`, which writes each id's index to `sort_order` and revalidates both `/habits` and `/today`. **Chose arrows over drag-and-drop deliberately** — the app is mobile-first and native HTML5 drag doesn't work on touch; drag would require a new dependency (`@dnd-kit`). Arrows work on touch + desktop with zero deps. The swap-point is documented in a `ponytail:` comment if drag is ever wanted.
...
7. **Admin surface — RESOLVED.** `(admin)` route group built with `is_admin` gate (`src/lib/admin/guard.ts`), audit-log writes on every cross-user read, and three pages: overview stats, users list, per-user overview, plus an audit-log tab. Deliberately no UI to promote/demote admins — bootstrap the first admin manually in Supabase SQL Editor: `update public.profiles set is_admin = true where user_id = '<your-user-id>';`. Everything under `/admin` uses the service-role client (via `src/lib/admin/queries.ts`) because it needs cross-user reads that anon+RLS can't do; the service-role containment comment now names that third file.
...
- ~~**Phase 9: Admin surface**~~ — **DONE.** See §9.7 for the resolved entry and the SQL to bootstrap the first admin.
